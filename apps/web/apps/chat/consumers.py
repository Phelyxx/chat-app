...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if self.should_disconnect(username):
            await self.close()
            return
    ...
    def should_disconnect(self, username):
        key = f'messages_per_minute_{username}'
        messages_per_minute = cache.get(key, default=0) + 1
        cache.set(key, messages_per_minute, timeout=60)
